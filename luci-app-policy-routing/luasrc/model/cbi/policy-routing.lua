local packageName = "policy-routing"
...
if sys.call("iptables -t mangle -L | grep -q PR_PREROUTING") == 0 then
	serviceRunning = true
...
local t = uci:get("policy-routing", "config", "supported_interface")
if not t then
	supportedIfaces = ""
...
t = uci:get("policy-routing", "config", "ignored_interface")
if not t then
	ignoredIfaces = ""
elseif type(t) == "table" then
...
status.template = "policy-routing/status"
...
-- General Options
config = m:section(NamedSection, "config", "policy-routing", translate("Configuration"))
...
if uci:get("policy-routing", "config", "webui_sorting") == "1" then
	p.sortable  = true
end
p.anonymous = true
p.addremove = true

if uci:get("policy-routing", "config", "webui_enable_column") == "1" then
	le = p:option(Flag, "enabled", translate("Enabled"))
	le.default = "1"
end

if uci:get_first("policy-routing", "policy", "comment") then
...
if uci:get("policy-routing", "config", "webui_protocol_column") == "1" then
	proto = p:option(ListValue, "proto", translate("Protocol"))
	proto:value("", "AUTO")
	proto.default = ""
	proto.rmempty = true
	enc = uci:get_list("policy-routing", "config", "webui_supported_protocol")
...
if uci:get("policy-routing", "config", "webui_chain_column") == "1" then
...
if uci:get("policy-routing", "config", "webui_show_ignore_target") == "1" then
	gw:value("ignore", "IGNORE")
end

dscp = m:section(NamedSection, "config", "policy-routing", translate("DSCP Tagging"), 
