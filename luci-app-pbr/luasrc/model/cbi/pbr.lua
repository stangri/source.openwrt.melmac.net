...
local t = uci:get("pbr", "config", "supported_interface")
if not t then
	supportedIfaces = ""
...
t = uci:get("pbr", "config", "ignored_interface")
if not t then
	ignoredIfaces = ""
elseif type(t) == "table" then
...
-- General Options
config = m:section(NamedSection, "config", "pbr", translate("Configuration"))
...
if uci:get("pbr", "config", "webui_enable_column") == "1" then
...
if uci:get("pbr", "config", "webui_protocol_column") == "1" then
	proto = p:option(ListValue, "proto", translate("Protocol"))
	proto:value("", "AUTO")
	proto.default = ""
	proto.rmempty = true
	enc = uci:get_list("pbr", "config", "webui_supported_protocol")
...
if uci:get("pbr", "config", "webui_chain_column") == "1" then
...
if uci:get("pbr", "config", "webui_show_ignore_target") == "1" then
	gw:value("ignore", "IGNORE")
end

dscp = m:section(NamedSection, "config", "pbr", translate("DSCP Tagging"), 
