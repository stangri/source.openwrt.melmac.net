...
for i=1, #devices do
  if board_name and devices[i][2] == board_name then
    ...
    if partition_one_label:find("LEDE") then partition_one_os = "LEDE" end
    if partition_one_label:find("OpenWrt") then partition_one_os = "OpenWrt" end
    if partition_one_label:find("Linksys") then partition_one_os = "Linksys" end
    if partition_two_label:find("LEDE") then partition_two_os = "LEDE" end
    if partition_two_label:find("OpenWrt") then partition_two_os = "OpenWrt" end
    if partition_two_label:find("Linksys") then partition_two_os = "Linksys" end
...
function index()
	...
	entry({"admin", "system", "reboot", "altcall"}, post("action_altreboot"))
...
function action_sysupgrade()
	...
	elseif step == 2 then
		...
		luci.template.render("admin_system/applyreboot", {
			...
			addr  = (#keep > 0) and "192.168.17.1" or nil
		})
...
function action_altreboot()
	if device_name and nixio.fs.access("/usr/sbin/fw_printenv") and nixio.fs.access("/usr/sbin/fw_setenv") then
	  if boot_envvar1 then local env1 = tonumber(luci.util.trim(luci.sys.exec("/usr/sbin/fw_printenv -n " .. boot_envvar1))) end
	  if boot_envvar2 then local env2 = luci.util.trim(luci.sys.exec("/usr/sbin/fw_printenv -n " .. boot_envvar2)) end
	  if boot_envvar1_partition_one and env1 == boot_envvar1_partition_one then luci.sys.call("/usr/sbin/fw_setenv " .. boot_envvar1 .. " " .. boot_envvar1_partition_two) end
	  if boot_envvar1_partition_two and env1 == boot_envvar1_partition_two then luci.sys.call("/usr/sbin/fw_setenv " .. boot_envvar1 .. " " .. boot_envvar1_partition_one) end
		if boot_envvar2_partition_one and env2 == boot_envvar2_partition_one then luci.sys.call("/usr/sbin/fw_setenv " .. boot_envvar1 .. " '" .. boot_envvar1_partition_two .. "'") end
		if boot_envvar2_partition_two and env2 == boot_envvar2_partition_two then luci.sys.call("/usr/sbin/fw_setenv " .. boot_envvar1 .. " '" .. boot_envvar1_partition_one .. "'") end
		luci.sys.reboot()
	end
end

function action_poweroff()
	if nixio.fs.access("/sbin/poweroff") then luci.sys.call("/sbin/poweroff") end
end
