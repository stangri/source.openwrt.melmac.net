...
|Advanced|iptables_rule_option|append/insert|append|Allows to specify the iptables parameter for rules: ```-A``` for ```append``` and ```-I``` for ```insert```. Append (default setting) keeps the order/priorities of the policies as listed in the config file and is generally speaking more compatible with other packages. Recommended to change to ```insert``` only to improve compatibility with the ```mwan3``` package.|
