...
- Super-fast due to the nature of supported block lists and parallel downloading/processing of the blocklists.
- Supports hosts files and domains lists for blocking.
- Everything is configurable from Web UI.
- Allows you to easily add your own domains to allowlist or blocklist.
- Allows you to easily add URLs to your own blocked hosts or domains lists to block/allowlist (just put allowlisted domains one per line in the file you're linking).
...
![screenshot](https://cdn.jsdelivr.net/gh/stangri/openwrt_packages@master/screenshots/simple-adblock/screenshot08-lists.png "Allowlist and Blocklist Management")
...
If you want to include some local files as the blocklists or allowlists, you need to install ```curl```:
...
You can also use Web UI to add individual domains to be blocked or allowlisted.
...
||allowlist_domain|list/string||List of white-listed domains.|
||allowlist_domains_url|list/string||List of URL(s) to text files containing white-listed domains. **Must** include either ```http://``` or ```https://``` (or, if ```curl``` is installed the ```file://```) prefix. Useful if you want to keep/publish a single white-list for multiple routers.|
||blocklist_domains_url|list/string||List of URL(s) to text files containing black-listed domains. **Must** include either ```http://``` or ```https://``` (or, if ```curl``` is installed the ```file://```) prefix.|
||blocklist_hosts_url|list/string||List of URL(s) to [hosts files](https://en.wikipedia.org/wiki/Hosts_(file)) containing black-listed domains. **Must** include either ```http://``` or ```https://``` (or, if ```curl``` is installed the ```file://```) prefix.|
